...
// Header for non-volatile memory storage
...
class Nvs
{
    const char* const   _LogTag{nullptr};
    nvs_handle_t        handle{};
...
public:
    ...
    Nvs(const char* const partition_name = "nvs") :
        _LogTag{partition_name},
        partition_name{partition_name} { nvs_flash_init_partition(partition_name); }
